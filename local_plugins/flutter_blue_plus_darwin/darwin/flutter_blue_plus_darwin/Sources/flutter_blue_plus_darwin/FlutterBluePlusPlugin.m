                              ...
                              error:(NSError *)error
{
    ...
    // --- SIPNUDGE WIDGET PATCH ---
    // Broadcast the 30-day hydration payload to AppDelegate so the widget can
    // be updated immediately, even if the Dart isolate is currently frozen.
    if ([[characteristic.UUID UUIDString] localizedCaseInsensitiveCompare:@"6E400006-B5A3-F393-E0A9-E50E24DCCA9E"] == NSOrderedSame) {
        if (characteristic.value) {
            NSString *payload = [[NSString alloc] initWithData:characteristic.value encoding:NSUTF8StringEncoding];
            if (payload) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"SipnudgeBLEDataReceived"
                                                                    object:nil
